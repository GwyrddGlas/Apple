...
float fbm5(vec3 v) {
    ...
    return result / (1.0 + 0.5 + 0.25 + 0.125 + 0.0625);
}
...
// Ensure to implement this simplex noise function
float simplex(vec3 v) {
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    vec2 uv = screen_coords.xy / iResolution;
    float noise = getNoise(vec3(uv * noiseScale, iTime * noiseTimeScale));
    noise = pow(noise, 4.0) * 3.0; // More contrast
    return vec4(noise * 0.3, noise * 0.0, noise * 0.0, 1.0); // Orange-ish tint
}